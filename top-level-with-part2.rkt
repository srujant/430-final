...
(define (correct? e env) 
    ...
    (define ((fun? env) e)
      ...
      (if (hash-has-key? env e) (let ([funs (hash-ref env e)]) (match funs [`(lambda ,x ,e) #t] [(? prim?) #t] [else (begin (display env) (display e) (raise "Not a function being applied to."))] )) (raise "Using variable that is not initialized."))]))
    ...
    (match e
      ...
      (if (hash-has-key? env fun) (let ([funs (hash-ref env fun)]) (match funs [`(lambda ,x ,e) (if (= (length x) (length es)) #t (if (< (length x) (length es)) (raise "Too many arguments given") (if (> (length x) (length es)) (raise "Too few arguments given") void)))] [(? prim?) #t] [else (begin (display e) (raise "Not a function being applied to."))] )) (raise "Using variable that is not initialized."))]) (begin (display e) (raise "Not a function being applied to.")))]
      [else #f]
      ))
